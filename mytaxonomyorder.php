...
function mytaxonomyorder_init() {
	...
	{   
		if (function_exists('add_management_page'))
			add_management_page(__('My Taxonomy Order','mytaxonomyorder'), __('My Taxonomy Order','mytaxonomyorder'), 4,'mytaxonomyorder','mytaxonomyorder');
	}
	
	function mytaxonomyorder_js_libs() {
		if ( $_GET['page'] == "mytaxonomyorder" )
		{	
	...
	function mytaxonomyorder()
	{
		...
		if(isset($_GET['hideNote']))
			update_option('mytaxonomyorder_hideNote', '1');
		...
		{  
			...
			$success = '<div id="message" class="updated fade"><p>'. __('Categories updated successfully.', 'mytaxonomyorder').'</p></div>';
		}
	...
	?>
		...
		if (get_option("mytaxonomyorder_hideNote") != "1")
			{	?>
				<div class="updated">
					<strong><p><?php _e('If you like my plugin please consider donating. Every little bit helps me provide support and continue development.','mytaxonomyorder'); ?> <a href="http://geekyweekly.com/gifts-and-donations"><?php _e('Donate', 'mytaxonomyorder'); ?></a>&nbsp;&nbsp;<small><a href="edit.php?page=mytaxonomyorder&hideNote=true"><?php _e('No thanks, hide this', 'mytaxonomyorder'); ?></a></small></p></strong>
				</div>
			<?php
			}?>
	...
	function mytaxonomyorder_loadtranslation() {
		load_plugin_textdomain('mytaxonomyorder', PLUGINDIR.'/'.dirname(plugin_basename(__FILE__)), dirname(plugin_basename(__FILE__)));
	}
